...
# ---------------------------------------------------------------------------
# Quy tac gia bat dong san — NGUON DUY NHAT cua hai ty le nay.
#
# Chi con 2 truong nhap tay: price_exclude_land_tax va land_tax. Con lai suy ra:
#
...
#
# product.discount.config dung lai dung hai helper nay o ca ba cho dung lai thap
# gia (_compute_percent_recalc, _chain_percent_recalc, get_schedule_discount_context)
# de gia san pham va gia sau chiet khau khong bao gio lech ty le.
# ---------------------------------------------------------------------------
VAT_RATE = 0.10
MAINTENANCE_RATE = 0.005
MAINTENANCE_ROUNDING = -3  # quy bao tri lam tron ve boi 1.000 dong
...
class ProductTemplate(models.Model):
    ...
    price_include_land_tax = fields.Monetary(
        ...
        help='= Giá nhà chưa bao gồm thuế SDĐ + Thuế sử dụng đất. Tự động tính, '
             'vẫn sửa tay được; sẽ bị tính lại khi hai trường nguồn thay đổi.',
    ...
    # của sản phẩm thường trong toàn hệ thống. Thay vào đó đồng bộ có điều kiện
    # trong create/write, chỉ chạm tới sản phẩm thực sự có nhập giá BĐS.
    #
    # Bản cũ ghi list_price ngay bên trong compute của price_include_land_tax.
    # Đó là lý do giá vừa sửa bị nhảy về giá cũ khi bấm lưu — mọi lần giá gốc
    # được tính lại đều ghi đè list_price trong cùng một lần lưu.
    # ------------------------------------------------------------------
    ...
    def _get_real_estate_list_price(self):
        self.ensure_one()
        return self.price_include_land_tax + self.vat_tax + self.maintenance_fee

    def _sync_real_estate_list_price(self):
        """Ghi lại list_price = Giá gồm TSDĐ + VAT + Quỹ bảo trì.

        CHỈ gọi khi lần ghi đó KHÔNG kèm list_price — người dùng gõ tay giá niêm
        yết thì số của họ được giữ nguyên (xem create/write bên dưới).

        Chỉ ghi khi số thực sự lệch. Lần write lồng bên trong chỉ chứa
        'list_price' — không nằm trong _REAL_ESTATE_PRICE_INPUTS — nên không
        kích hoạt lại hàm này: không có đệ quy.

        Sản phẩm không nhập giá BĐS thì bỏ qua hoàn toàn, giá bán do người dùng
    ...
    def create(self, vals_list):
        ...
        # Bản ghi nào tự mang theo list_price thì tôn trọng số đó, không ghi đè.
        to_sync = self.browse()
        for product, vals in zip(products, vals_list):
            if 'list_price' not in vals:
                to_sync |= product
        to_sync._sync_real_estate_list_price()
        return products

    def write(self, vals):
        res = super().write(vals)
        # list_price có mặt trong vals nghĩa là giá trị đã được ấn định:
        #   - form gửi lên số do _onchange_real_estate_list_price vừa tính, hoặc
        #   - người dùng gõ tay đè lên.
        # Cả hai trường hợp đều giữ nguyên. Chỉ tự tính khi lần ghi này không
        # nhắc tới list_price (import Excel, ghi từ code, sửa qua ORM...).
        if 'list_price' not in vals and any(
                field in vals for field in self._REAL_ESTATE_PRICE_INPUTS):
    ...
    def action_recalculate_prices(self):
        ...
        đưa về lại công thức. price_include_land_tax và price_per_m2 là compute
        nên tự theo sau, không cần gán tay.
        """
        for product in self:
            product.price_include_land_tax = (
                product.price_exclude_land_tax + product.land_tax
